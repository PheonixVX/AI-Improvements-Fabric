...
public class ModifierLayer
{
	...
	{
		final GoalSelector goalSelector = combatAI ? ((MobEntityDuck) entity).getTargetSelector() : ((MobEntityDuck) entity).getGoalSelector();

		final Set<Goal> goalsToRemove = new HashSet<>();
		for (PrioritizedGoal prioritizedGoal : ((GoalSelectorDuck) goalSelector).getGoals())
		{
